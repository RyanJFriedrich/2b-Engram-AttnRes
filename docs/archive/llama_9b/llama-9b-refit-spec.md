...
- **Frozen from step 0 (owner decision 2026-09-03):** embed_tokens and lm_head are *not trained* in the initial phase — the new interior (SWA/global/gather + AttnRes + Engram) must learn to bridge the fixed donor input/output representations. Rationale: the donor matrices are already well-trained; letting them move under a loss dominated by the cold interior's mismatch just pumps noise into them. **This freeze is explicitly TEMPORARY: unfreeze only after the model has seen a lot more tokens** (owner-gated decision, revisited when the interior has stabilized); `freeze_embeddings: false` in the run config is the mechanism when that day comes.
