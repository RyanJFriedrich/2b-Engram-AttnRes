...
## §9. Known watch-items in the landed code (2026-09-01)

Implementation choices that are correct as landed but carry a flagged risk or
a known-delicate spot. Not exclusions — these exist; do not "fix" them
silently, but do watch them at bring-up.

- **Sink softmax denominator (fp32, max-stable).** The learned-sink attention
  softmax computes its denominator in fp32 with explicit max stabilization
  (`train/src/model/refit.py`, sink path) — this is the numerically delicate
  spot of the v2.0 attention and it is deliberately kept out of bf16/fp8
  autocast behavior. If attention logits ever look unstable at scale, look
  here first; it is a known issue area, not an oversight.
- **FP8 per-tensor scaling.** The §4 FP8 compute path uses per-tensor dynamic
  scaling (e4m3 fwd / e5m2 grads), not rowwise/Transformer-Engine-style
  scaling — chosen for simplicity per the spec's minimal §4 recipe. Possible
  issue: at 9B scale per-tensor scales are keyed to the tensor amax, so
  outlier-heavy activations compress the usable grid for everything else.
  The bring-up acceptance test is the FP8-vs-bf16 loss-curve overlay; if it
  shows a real gap, the upgrade path is rowwise scales — not tuning.
- **torch.compile x anneal ablation.** `torch_compile: true` compiles one
  static graph per run (fixed [B, 8192] batches). The §7.2 anneal ablation
  mutates attention window/theta BETWEEN steps, which forces a recompile per
  schedule change — do not combine the two (anneal is off by default; compile
  is off in the bring-up smokes). Not a bug; a recorded non-goal combo.
